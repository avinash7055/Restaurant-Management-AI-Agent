...
def check_availability(date: str = None):
    ...
    Returns:
        Dictionary of available tables by date and time
    """
    db = get_db()
...
def make_reservation(customer_name: str, date: str, time: str, party_size: int):
    ...
    if reservation:
        ...
        return {
            ...
            "message": f"Reservation confirmed for {customer_name} on {date} at {time} for {party_size} people. Your reservation ID is {reservation.id}. You can view your reservation details in the Reservations section of the navigation bar."
        }
    else:
        return {
            "success": False,
            "message": f"Sorry, no tables are available on {date} at {time}. Please try another time or date."
        }
